...
func NewRepository[T IBaseEnity](db *sql.DB, entity T) *Repository[T] {
...
func (r *Repository[T]) UpdateMany(ctx context.Context, entity *map[string]any, option *UpdateOption) (int64, *response.AppError) {
	...
	return converError(result.RowsAffected())
}
...
func (r *Repository[T]) DeleteMany(ctx context.Context, option *UpdateOption) (int64, *response.AppError) {
	...
	result, err := r.db.ExecContext(ctx, query, option.Where.Params...)
	if err != nil {
		return 0, response.DatabaseError(err)
	}
...
func (r *Repository[T]) PaginationQuery(ctx context.Context, option *FindOption) ([]T, uint64, *response.AppError) {
	...
	count := r.db.QueryRowContext(ctx, countQuery, option.Where.Params...)
	...
	rows, err := r.db.QueryContext(ctx, query, option.Where.Params...)
	if err != nil {
		return nil, 0, response.DatabaseError(err)
	}
...
func (r *Repository[T]) FindOne(ctx context.Context, option *FindOption) (*T, *response.AppError) {
	...
	row, err := r.db.QueryContext(ctx, query, option.Where.Params...)
	if err != nil {
		return nil, response.DatabaseError(err)
	}
...
func converError[T any](data T, err error) (T, *response.AppError) {
	if err != nil {
		return data, response.DatabaseError(err)
	}
